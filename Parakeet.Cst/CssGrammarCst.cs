...
namespace Ara3D.Parakeet.Cst.CssGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = Attrib ::= ('['+WS+_TRUE_+Ident+AttribOperator+WS+((AttribValue+WS))?+WS+']'+WS)
    ...
    /// <summary>
    /// Rule = AttribOperator ::= ('='|Includes|DashMatch)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = CharSet ::= (CharSetSym+String+';')
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = CombinedSelector ::= (((Combinator+WS))?+(Selector+WS))
    ...
    /// <summary>
    /// Rule = Content ::= ((RuleSet+WS)|(MediaList+WS)|(Page+WS))
    ...
    /// <summary>
    /// Rule = Contents ::= ((Content+WS+CommentParts))*
    /// Nodes = (Content)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = Declaration ::= (Property+WS+Symbol(':')+Expr+WS+((Prio+WS))?)
    ...
    /// <summary>
    /// Rule = Declarations ::= (((Selector+WS+WS+((Symbol(';')+Selector+WS+WS))*)+(Symbol(';'))?))?
    /// Nodes = (Selector)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = Expr ::= (((Term+WS)+WS+((((Operator+WS))?+(Term+WS)+WS))*)+(((Operator+WS))?)?)
    ...
    /// <summary>
    /// Rule = Function ::= (Ident+'('+WS+_TRUE_+((Expr+WS))?+WS+')'+WS)
    /// Nodes = (Expr)?
    /// </summary>
    ...
    /// <summary>
    /// Rule = HexColor ::= (('#'+(HexDigit){6,6})|(HexDigit){3,3})
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Imports ::= ((Import+WS+CommentParts))*
    /// Nodes = (Import)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = MediaList ::= (((Medium+WS+WS+((Comma+Medium+WS+WS))*)+(Comma)?))?
    /// Nodes = (Medium)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = Page ::= (PageSym+WS+((PseudoPage+WS))?+PageDeclarations+WS)
    ...
    /// <summary>
    /// Rule = PageDeclarations ::= ('{'+WS+_TRUE_+(((Declaration+WS+WS+((Symbol(';')+Declaration+WS+WS))*)+(Symbol(';'))?))?+WS+'}'+WS)
    /// Nodes = (Declaration)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = Prio ::= ('!'+((WS|Comment))*+_UNKNOWN_)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = RuleSet ::= (Selectors+WS+'{'+WS+_TRUE_+Declarations+WS+WS+'}'+WS)
    ...
    /// <summary>
    /// Rule = Selector ::= (SimpleSelector+WS+((CombinedSelector+WS))?)
    ...
    /// <summary>
    /// Rule = SelectorPart ::= (Hash|(Class+WS)|(Attrib+WS)|(Pseudo+WS))
    ...
    /// <summary>
    /// Rule = Selectors ::= (((Selector+WS+WS+((Comma+Selector+WS+WS))*)+(Comma)?))?
    /// Nodes = (Selector)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = SimpleSelector ::= ((ElementName+WS+((SelectorPart+WS))*)|((SelectorPart+WS))+)
    ...
    /// <summary>
    /// Rule = StyleSheet ::= (((CharSet+WS))?+((S|Cdo|Cdc))*+Imports+WS+Contents+WS)
    ...
    /// <summary>
    /// Rule = Term ::= (((UnaryOperator+WS))?+(Percentage|Length|Ems|Exs|Angle|Time|Freq|String|Ident|Number|Uri|(HexColor+WS)|(Function+WS)))
