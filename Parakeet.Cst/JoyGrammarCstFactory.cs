...
namespace Ara3D.Parakeet.Cst.JoyGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IJoyCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IJoyCstNode, ParserTreeNode>();
        public IJoyCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IJoyCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
