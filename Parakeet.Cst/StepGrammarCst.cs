...
namespace Ara3D.Parakeet.Cst.StepGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = Aggregate ::= ('('+WS+_RECOVER_+((((Value+WS)+WS+((Comma+(Value+WS)+WS))*)+(Comma)?))?+WS+')'+WS)
    /// Nodes = (Value)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = AttributeValues ::= ('('+WS+_RECOVER_+((((Value+WS)+WS+((Comma+(Value+WS)+WS))*)+(Comma)?))?+WS+')'+WS)
    /// Nodes = (Value)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = ComplexEntityData ::= ('('+WS+_TRUE_+((SimpleEntityData+WS))++WS+')'+WS)
    ...
    /// <summary>
    /// Rule = DataEntry ::= (InstanceName+WS+_RECOVER_+'='+WS+EntityData+WS+';')
    ...
    /// <summary>
    /// Rule = DataSection ::= (DataBegin+WS+_RECOVER_+((DataEntry+WS))*+EndSection+WS)
    ...
    /// <summary>
    /// Rule = EntityData ::= ((ComplexEntityData+WS)|(SimpleEntityData+WS))
    ...
    /// <summary>
    /// Rule = File ::= (Magic+WS+_RECOVER_+Header+WS+DataSection+WS)
    ...
    /// <summary>
    /// Rule = Header ::= (BeginHeader+WS+((!((EndSection+WS))+AnyChar))*+(EndSection+WS))
    ...
    /// <summary>
    /// Rule = InstanceName ::= ('#'+_RECOVER_+Integer)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Magic ::= "ISO\x2D10303\x2D21\x3B"
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Number ::= (Integer+(DecimalPart)?+(ExponentPart)?)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = SimpleEntityData ::= (EntityDataName+WS+_RECOVER_+AttributeValues+WS)
    ...
    /// <summary>
    /// Rule = String ::= ('''+WS+_TRUE_+(StringChar)*+WS+'''+WS)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Value ::= ((InstanceName+WS)|(Symbol+WS)|(Aggregate+WS)|(String+WS)|(Number+WS)|(UnsetAttributeValue+WS)|(RedeclaredAttribute+WS)|(SimpleEntityData+WS))
