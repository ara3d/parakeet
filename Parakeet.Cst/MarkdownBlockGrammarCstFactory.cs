...
namespace Ara3D.Parakeet.Cst.MarkdownBlockGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IMarkdownBlockCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IMarkdownBlockCstNode, ParserTreeNode>();
        public IMarkdownBlockCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IMarkdownBlockCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
                ...
                case "H1Underline": return new CstH1Underline(node.Children.Select(Create).ToArray());
                case "H2Underline": return new CstH2Underline(node.Children.Select(Create).ToArray());
                ...
                case "Indents": return new CstIndents(node.Children.Select(Create).ToArray());
                case "IndentsOrQuoteMarkers": return new CstIndentsOrQuoteMarkers(node.Children.Select(Create).ToArray());
                ...
                case "QuoteMarker": return new CstQuoteMarker(node.Contents);
