...
namespace Ara3D.Parakeet.Cst.MarkdownInlineGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = AltText ::= ('['+WS+_TRUE_+((!(']')+AnyChar))*+WS+']'+WS)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Bold ::= (("\x2A\x2A"+((!("\x2A\x2A")+(InnerText+WS)))*+"\x2A\x2A")|("\x5F\x5F"+((!("\x5F\x5F")+(InnerText+WS)))*+"\x5F\x5F"))
    /// Nodes = (InnerText)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = BoldAndItalic ::= (("\x2A\x2A\x2A"+((!("\x2A\x2A\x2A")+(InnerText+WS)))*+"\x2A\x2A\x2A")|("\x5F\x5F\x5F"+((!("\x5F\x5F\x5F")+(InnerText+WS)))*+"\x5F\x5F\x5F"))
    /// Nodes = (InnerText)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = Email ::= ((EmailChar)*+'@'+(EmailChar)*)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = EscapedChar ::= ('\'+[\x21\x23\x28-+\x2D\x2E<>\x5B-\x5D`\x7B-\x7D])
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = HtmlTag ::= ('<'+_RECOVER_+((!('>')+AnyChar))*+'>')
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Img ::= ('!'+AltText+WS+'('+WS+_TRUE_+Url+WS+WS+((UrlTitle+WS))?+WS+')'+WS)
    ...
    /// <summary>
    /// Rule = InnerText ::= ((BoldAndItalic+WS)|(Strikethrough+WS)|(Bold+WS)|(Italic+WS)|(Code+WS)|(Link+WS)|(Img+WS)|(EmailLink+WS)|(UrlLink+WS)|(HtmlTag+WS)|(EscapedChar+WS)|(PlainText+WS)|AnyChar)
    ...
    /// <summary>
    /// Rule = Italic ::= (('*'+((!('*')+(InnerText+WS)))*+'*')|('_'+((!('_')+(InnerText+WS)))*+'_'))
    /// Nodes = (InnerText)*
    /// </summary>
    ...
    /// <summary>
    /// Rule = Link ::= ('['+LinkedText+WS+']'+WS+'('+WS+_TRUE_+Url+WS+WS+((UrlTitle+WS))?+WS+')'+WS)
    ...
    /// <summary>
    /// Rule = PlainText ::= ((!([\x21\x2A<>\x5B-\x5D\x5F`~])+AnyChar))+
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = PlainTextUrl ::= ("http\x3A\x2F\x2F"+(UrlChar)*)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = UrlLink ::= (('<'+Url+WS+'>')|(PlainTextUrl+WS))
    ...
    /// <summary>
    /// Rule = UrlTitle ::= ('"'+WS+_TRUE_+((!('"')+("\x5C\x22"|AnyChar)))*+'"'+WS+'"'+WS)
    /// Nodes = 
    /// </summary>
