...
namespace Ara3D.Parakeet.Cst.PlatoGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IPlatoCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IPlatoCstNode, ParserTreeNode>();
        public IPlatoCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IPlatoCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
