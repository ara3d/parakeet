...
namespace Ara3D.Parakeet.Cst.PlatoGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = LeafExpression ::= ((LambdaExpr+WS)|(CastExpression+WS)|(ParenthesizedExpression+WS)|(ArrayExpression+WS)|(ThrowExpression+WS)|(Literal+WS)|(TypeOf+WS)|(NameOf+WS)|(Default+WS)|(NewOperation+WS)|(StringInterpolation+WS)|(Identifier+WS))
    ...
    /// <summary>
    /// Rule = Type ::= ((UniqueKeyword)?+Keyword('type')+_RECOVER_+Identifier+WS+TypeParameterList+WS+ImplementsList+WS+'{'+WS+_RECOVER_+((FieldDeclaration+WS))*+WS+'}'+WS)
    /// Nodes = ((UniqueKeyword)?+Identifier+TypeParameterList+ImplementsList+(FieldDeclaration)*)
    /// </summary>
    ...
    /// <summary>
    /// Rule = UniqueKeyword ::= (Keyword('unique'))
    /// Nodes =
    /// </summary>
