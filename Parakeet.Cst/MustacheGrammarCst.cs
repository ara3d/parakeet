...
namespace Ara3D.Parakeet.Cst.MustacheGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = Block ::= ((InvertedSection+(Spaces)?)|(Section+(Spaces)?)|(Comment+(Spaces)?)|(Partial+(Spaces)?)|(Variable+(Spaces)?)|(UnescapedVariable+(Spaces)?)|(PlainText+(Spaces)?))
    ...
    /// <summary>
    /// Rule = Comment ::= (Start+'!'+_RECOVER_+(Space)*+Key+(Spaces)?+End)
    /// Nodes = Key
    /// </summary>
    ...
    /// <summary>
    /// Rule = EndSection ::= (Start+'/'+_RECOVER_+(Space)*+Key+(Spaces)?+End)
    /// Nodes = Key
    /// </summary>
    ...
    /// <summary>
    /// Rule = InvertedSection ::= (StartInvertedSection+(Spaces)?+_RECOVER_+(Content+(Spaces)?)+EndSection+(Spaces)?)
    ...
    /// <summary>
    /// Rule = Partial ::= (Start+'>'+_RECOVER_+(Space)*+Key+(Spaces)?+End)
    /// Nodes = Key
    /// </summary>
    ...
    /// <summary>
    /// Rule = PlainText ::= ((!((Start|EndOfInput))+AnyChar))*
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = RestOfLine ::= (((SpaceOrTab)*+NewLine)|EndOfInput)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = Section ::= (StartSection+(Spaces)?+_RECOVER_+(Content+(Spaces)?)+EndSection+(Spaces)?)
    ...
    /// <summary>
    /// Rule = StartInvertedSection ::= (Start+'^'+_RECOVER_+(Space)*+Key+(Spaces)?+End+RestOfLine+(Spaces)?)
    ...
    /// <summary>
    /// Rule = StartSection ::= (Start+'#'+_RECOVER_+(Space)*+Key+(Spaces)?+End+RestOfLine+(Spaces)?)
    /// Nodes = (Key+RestOfLine)
    /// </summary>
    ...
    /// <summary>
    /// Rule = UnescapedVariable ::= ((Start+'{'+_RECOVER_+(Space)*+Key+(Spaces)?+End+('}')?)|(Start+'&'+_RECOVER_+(Space)*+Key+(Spaces)?+End))
    /// Nodes = (Key|Key)
    /// </summary>
    ...
    /// <summary>
    /// Rule = Variable ::= (Start+!([\x21\x23\x26\x2F<^\x7B])+_RECOVER_+(Space)*+Key+(Spaces)?+End)
    /// Nodes = Key
    /// </summary>
