...
namespace Ara3D.Parakeet.Cst.XmlGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IXmlCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IXmlCstNode, ParserTreeNode>();
        public IXmlCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IXmlCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
