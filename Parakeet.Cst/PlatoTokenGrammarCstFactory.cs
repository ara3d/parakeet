...
namespace Ara3D.Parakeet.Cst.PlatoTokenGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IPlatoTokenCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IPlatoTokenCstNode, ParserTreeNode>();
        public IPlatoTokenCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IPlatoTokenCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
