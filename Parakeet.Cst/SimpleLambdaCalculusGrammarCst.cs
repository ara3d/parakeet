...
namespace Ara3D.Parakeet.Cst.SimpleLambdaCalculusGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = Abstraction ::= ('('+Parameter+(Spaces)?+'.'+Term+(Spaces)?+')')
    ...
    /// <summary>
    /// Rule = Application ::= ('('+Term+(Spaces)?+(Spaces)?+Term+(Spaces)?+')')
    /// Nodes = (Term+Term)
    /// </summary>
    ...
    /// <summary>
    /// Rule = InnerTerm ::= ((Variable+(Spaces)?)|(Abstraction+(Spaces)?)|(Application+(Spaces)?))
