...
namespace Ara3D.Parakeet.Cst.MarkdownInlineGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IMarkdownInlineCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IMarkdownInlineCstNode, ParserTreeNode>();
        public IMarkdownInlineCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IMarkdownInlineCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
