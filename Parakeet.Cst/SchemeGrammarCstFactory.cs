...
namespace Ara3D.Parakeet.Cst.SchemeGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<ISchemeCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<ISchemeCstNode, ParserTreeNode>();
        public ISchemeCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public ISchemeCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
