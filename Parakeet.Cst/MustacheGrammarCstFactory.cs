...
namespace Ara3D.Parakeet.Cst.MustacheGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IMustacheCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IMustacheCstNode, ParserTreeNode>();
        public IMustacheCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IMustacheCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
