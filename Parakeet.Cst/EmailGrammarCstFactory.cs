...
namespace Ara3D.Parakeet.Cst.EmailGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<IEmailCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<IEmailCstNode, ParserTreeNode>();
        public IEmailCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public IEmailCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
