...
namespace Ara3D.Parakeet.Cst.MarkdownBlockGrammarNameSpace
{
    ...
    /// <summary>
    /// Rule = Block ::= ((CodeBlock+WS)|(Comment+WS)|(Line+WS))
    ...
    /// <summary>
    /// Rule = BlockQuotedLine ::= (Indents+WS+QuoteMarker+WS+RestOfLine+WS)
    ...
    /// <summary>
    /// Rule = CodeBlock ::= (CodeBlockDelimiter+_RECOVER_+CodeBlockLang+WS+CodeBlockText+WS+CodeBlockDelimiter)
    ...
    /// <summary>
    /// Rule = CodeBlockLang ::= (((Identifier+WS))?+_RECOVER_+BlankLine+WS)
    ...
    /// <summary>
    /// Rule = CodeBlockText ::= ((!(CodeBlockDelimiter)+AnyChar))*
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = H1Underline ::= (IndentsOrQuoteMarkers+WS+WS+('='){2,2147483647}+_RECOVER_+EatWsToNextLine)
    ...
    /// <summary>
    /// Rule = H2Underline ::= (IndentsOrQuoteMarkers+WS+WS+('-'){2,2147483647}+_RECOVER_+EatWsToNextLine)
    /// Nodes = IndentsOrQuoteMarkers
    /// </summary>
    ...
    /// <summary>
    /// Rule = Heading ::= ((HeadingWithOperator+WS)|(HeadingUnderlined+WS))
    ...
    /// <summary>
    /// Rule = HeadingUnderlined ::= (TextLine+WS+((H1Underline+WS)|(H2Underline+WS)))
    ...
    /// <summary>
    /// Rule = HeadingWithOperator ::= (HeadingOperator+WS+TextLine+WS)
    ...
    /// <summary>
    /// Rule = Indent ::= (Tab|"\x20\x20\x20\x20")
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = IndentsOrQuoteMarkers ::= (((Indent+WS)|(QuoteMarker+WS)))*
    ...
    /// <summary>
    /// Rule = Line ::= ((Heading+WS)|(HorizontalLine+WS)|(UnorderedListItem+WS)|(OrderedListItem+WS)|(BlockQuotedLine+WS)|(BlankLine+WS)|(NonEmptyTextLine+WS))
    ...
    /// <summary>
    /// Rule = NonEmptyTextLine ::= (!(NewLine)+AnyChar+AnyCharUntilNextLine)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = OrderedListItem ::= (Indents+WS+WS+(Digit)++'.'+WS+_RECOVER_+TextLine+WS)
    ...
    /// <summary>
    /// Rule = TextLine ::= (((!((NewLine|EndOfInput))+AnyChar))*+(NewLine)?)
    /// Nodes = 
    /// </summary>
    ...
    /// <summary>
    /// Rule = UnorderedListItem ::= (Indents+WS+WS+[\x2A+\x2D]+WS+_RECOVER_+TextLine+WS)
    /// Nodes = (Indents+TextLine)
    /// </summary>
