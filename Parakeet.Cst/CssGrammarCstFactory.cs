...
namespace Ara3D.Parakeet.Cst.CssGrammarNameSpace
{
    public class CstNodeFactory : INodeFactory
    {
        ...
        public Dictionary<ICssCstNode, ParserTreeNode> Lookup { get;} = new Dictionary<ICssCstNode, ParserTreeNode>();
        public ICssCstNode Create(ParserTreeNode node)
        {
            var r = InternalCreate(node);
            Lookup.Add(r, node);
            return r;
        }
        public ICssCstNode InternalCreate(ParserTreeNode node)
        {
            switch (node.Type)
            {
